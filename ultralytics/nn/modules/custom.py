...
class DSConv(nn.Module):
    ...
    This convolution adapts its sampling locations to follow snake-like patterns, making it ideal for detecting objects
    with irregular edges like potholes.
    ...
    def _deform_input(self, x, offset):
        ...
        grid_y, grid_x = torch.meshgrid(
            torch.linspace(-1, 1, H, device=x.device), torch.linspace(-1, 1, W, device=x.device), indexing="ij"
        )
...
class DySnakeConv(nn.Module):
    ...
    Combines x-axis and y-axis snake convolutions with a standard convolution for comprehensive feature extraction of
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        self.conv_0 = nn.Sequential(nn.Conv2d(c1, c_, k, padding=autopad(k), bias=False), nn.BatchNorm2d(c_), nn.GELU())
        ...
        # Fusion convolution
        self.conv_fusion = nn.Sequential(nn.Conv2d(c_ * 3, c2, 1, bias=False), nn.BatchNorm2d(c2), nn.GELU())
...
class SimAM(nn.Module):
    ...
    SimAM computes 3D attention weights without adding any learnable parameters. It evaluates the importance of each
    neuron based on an energy function, enabling the model to focus on important features like pothole regions.
...
class C3k2_DSConv(nn.Module):
    ...
    Enhanced C3k2 that uses DSConv bottlenecks for better boundary detection of irregular shapes like potholes.
    """
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(Bottleneck_DSConv(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class C3k2_SimAM(nn.Module):
    ...
    Uses SimAM attention after bottleneck operations to help the model focus on important features like pothole regions.
    """
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(Bottleneck_GELU(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class SPPF_SimAM(nn.Module):
    ...
    Enhanced SPPF module that applies SimAM attention for better feature focus in multi-scale pooling.
    """
